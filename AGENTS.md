...
| `npx tsx tools/execute.ts <feature-id>` | Determine which packets to implement next |
| `npx tsx tools/complete.ts <packet-id>` | After implementation, before committing |
| `npx tsx tools/report.ts <feature-id>` | Produce QA report after all packets complete (pipe assessments JSON to stdin) |
...
3. If a feature is active, run `npx tsx tools/execute.ts <feature-id>`
