...
// Approval-semantics split tests moved to tools/test/spec_runner.test.ts
// in the backlog cleanup pass — they exercise per-spec runner behavior
// (`tools/pipeline/orchestrator/spec_runner.ts`), not the orchestrator's
// driver loop (multi-spec sequencing / cycles / missing-deps / failure
// propagation), which stays here.
