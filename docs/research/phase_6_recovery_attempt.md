---
name: The Phase 6 recovery attempt that wasn't — bounded-iteration revert lesson on cross-cutting layer integration
description: Bounded-iteration revert lesson recorded 2026-05-05. Phase 6 of `specs/single-entry-pipeline.md` was scoped as the recovery layer per [recovery_recipes_not_dsl.md](../decisions/recovery_recipes_not_dsl.md) — 8 failure scenarios with scenario-keyed recipes, per-scenario per-packet retry budgets, 4 new event types (`recovery.attempt_started/succeeded/exhausted/escalated`), wiring at every `invokeAgent` site and `complete.ts` boundary, plus a pure/I-O/orchestration split mirroring Phases 5.5 and 5.7. Three rounds of codex GPT-5.5 review surfaced increasingly deep correctness gaps. Round 1: recipe action labels propagated through the system but the actions themselves never executed (CLAUDE.md §3.5 facade pattern — tests pinned labels, not behavior). Round 2: actions wired correctly but `StaleBranch` remained production-unreachable and QA-completion `BuildFailed` remediation passed the QA packet to `buildDevPrompt` instead of the dev packet referenced by `packet.verifies`. Round 3 verdict: RECOMMEND-REVERT. Codex found the architectural issue beneath the fixes: `StaleBranch` escalation at `request_review` was observable (events fired, escalation files written) but not controlling — the failed `runWithRecovery` result was only logged, then the develop-phase state machine advanced to review unconditionally and could still complete the packet through the bad fallthrough. CLAUDE.md §3.3 violation: failure must be both visible AND controlling. The lesson: when adding an orthogonal cross-cutting layer (recovery, tracing, auth) to an existing state machine, the integration IS the architecture, not the wiring; the cross-cutting layer's signals must be both observable (events fire, audits accumulate) AND controlling (the calling loop dispatches on them). The brief had listed "wire recovery into phases" as the last commit and named it wiring rather than integration; the recovery layer was correct in isolation, the test suite confirmed it, the production state machine ignored it. What's salvageable for Phase 6 (revised): pure types/classifier/recipes module, writeEscalation I/O, the 4 new event types, `runGitRebase` execution, BuildFailed dev-agent remediation, QA dev-packet resolution, cross-layer pattern drift test, boundary discipline, cap-vs-retry interaction order. What needs redesign: state-machine integration as a first-class concern (per-packet loops must stop on escalation, not log-and-continue), `runWithRecovery` returning a discriminator the caller matches on (`{ ok: true, value } | { escalated: true, reason }`), and end-to-end integration tests that assert post-escalation invariants (packet marked failed, no further transitions). Second case (after [ajv_migration_attempt.md](ajv_migration_attempt.md)) where the bounded-iteration model surfaced an architectural error in the brief itself. Original branch `phase-6-recovery` was force-deleted post-revert.
type: lesson
---
