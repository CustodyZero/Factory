...
description: Memory in factory operates at two scopes — per-packet workers stay stateless; the project carries learned memory across runs. Worker-scope follows the claw-code model; project-scope follows the claurst memdir model. Project-scope further subdivides into host-project memory (factory's contract with host projects, written to the host's tracked artifact tree; the host owns the loading mechanism) and factory-development memory (memdir-style symlink convention specific to this repo's own development; per-developer setup, NOT inherited by host projects). Memory categories when implemented: architectural facts, recurring failures, code patterns. Operator preferences are out of scope.
type: project
---
