---
name: Reviewer CLI exception — review.ts is the verdict channel, kept honest by ReviewDecisionMissing
description: The code-reviewer agent invokes `review.ts --approve` or `review.ts --request-changes` as the protocol channel for its verdict, in deliberate contrast to the planner, developer, rework, and QA prompts, which are explicitly instructed NOT to call lifecycle CLIs (the pipeline drives those transitions itself once the agent process exits). The reviewer is the exception because its output IS a structured verdict the pipeline must consume reliably to decide whether to advance to `complete` or loop back to `rework`, and there is no mechanical way to derive that verdict from an exit code — it lives in the model's judgement about the code. That makes the verdict exactly the kind of structured-output problem CLI-as-agent-protocol was designed to solve in [single_entry_pipeline.md](single_entry_pipeline.md). Stdout parsing was rejected (inherits the LLM structured-output fragility CLI-as-protocol exists to prevent); a dedicated verdict file was rejected (new protocol surface for one persona). The exception is kept honest by the `ReviewDecisionMissing` recovery scenario: when the reviewer exits 0 without invoking `review.ts`, the develop-phase closure synthesizes a failure carrying `REVIEW_DECISION_MISSING_MARKER`, the classifier matches by exact substring, and the recipe escalates with budget 0 — no silent force-approve. Without that path, the exception would be a facade; the prompt-exception and the failure-path are coupled by design. Decided 2026-05-11 during the post-Phase-8 convergence pass; load-bearing references in `tools/pipeline/prompts.ts:buildReviewPrompt`, `tools/pipeline/develop_phase.ts` (the post-review status check), and `tools/pipeline/recovery.ts` (the scenario, classifier branch, and escalate-only recipe).
type: project
---
