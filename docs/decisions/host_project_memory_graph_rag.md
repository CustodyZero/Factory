---
name: factory-host-project-memory-graph-rag
description: When the host-project memory write-side (deferred per memory_scope_split.md) is implemented, it will be a graph-based knowledge layer with typed nodes, typed edges, composite node weights, semantic+graph retrieval, and continuous consolidation — NOT a flat catalog of markdown notes. The source of truth is human-readable markdown in factory/memory/<category>/<id>.md; a derived SQLite+vector index makes retrieval fast. Extraction runs at pipeline end (best-effort, never fails the run); retrieval injects a typed MemoryContext block into planner and per-phase agent prompts; consolidation (clustering, contradiction detection, reinforcement, decay) is first-class. This doc records the architectural commitment. Specific implementation choices (embedding model, vector store dependency, retrieval-injection scope, consolidation cadence) are A-decisions deferred to 4-6 future implementation specs across 3 staging steps.
type: project
---
