---
name: Spec artifact model — human-authored markdown one layer above intents, with 1:1 spec → intent parity
description: A new artifact type — spec — sits one layer above the existing intent. Specs are human-authored markdown files at `specs/<spec-id>.md` with minimal YAML frontmatter (`id`, `title`, optional `depends_on`); the body is the authoritative content the planner reads. Each spec maps 1:1 to an intent; factory translates spec → intent at run time inside the pipeline runner (no separate "import" command, no mid-state where a spec exists without its intent). The intent remains the locked schema downstream tooling consumes, with one additive backward-compatible field added (`depends_on`, default `[]`); the spec is the source of truth, the intent is derived state regeneratable from disk. `depends_on` is the only spec-level sequencing primitive — cyclic dependencies are rejected at validation time. The post-Phase-8 convergence pass introduced a runtime governance gate that distinguishes spec-driven runs (`run.ts <spec-id>`, bypass the gate because human approval happened when the spec was written) from intent-driven runs against a hand-authored `intents/<id>.json` (must carry `status: approved`/`planned`/`delivered`, since the operator wrote the file). The split keeps spec-as-derived-state honest while restoring an explicit governance signal on the hand-authored fallback. Decided 2026-04-30; the spec-vs-intent approval-gate split was documented 2026-05-10. Informed by [factory_script_audit.md](../research/factory_script_audit.md).
type: project
---
