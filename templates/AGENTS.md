...
loop:
  ...
  2. Read the action kind in the output:
     - spawn_packets  → spawn agents for ready packets using the assigned persona, run `npx tsx .factory/tools/start.ts <packet-id>` for each assigned packet, complete each, go to 1
